...
class orientation_calc(Node):
    
    def __init__(self):
        super().__init__('orient_calc')
    ...
    def listener_callback(self, msg):
        ...
        d,e,f = LLtoUTM(44.663,-75.00)
        a,b,c = LLtoUTM(msg.latitude,msg.longitude)
        g = a-d
        h = b-e
        print(c)
        if counter == 0:
            coords[1] = h
            coords[2] = g
            z = input("Starting coordinates recieved. Drive the vehicle a couple meters forward, then press enter: ")
        elif counter == 1:
            coords[3] = h
            coords[4] = g
            th = getAngle(coords[1],coords[2],coords[3],coords[4])
            fi = open(input_file, 'r')
            fd = open(output_file, 'w')
            i = fi.readline()
            while (i != ""):
                j = i.find("initial_state:")
                if j == -1:
                    fd.write(i)
                else:
                    fd.write("    initial_state: [")
                    fd.write(repr(h))
                    fd.write(", ")
                    fd.write(repr(g))
                    fd.write(", 0.0,\n")
...
def main(args=None):
    ...
    orient_calc = orientation_calc()

    rclpy.spin(orient_calc)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    orient_calc.destroy_node()
    rclpy.shutdown()
